...
function renderQuestGroup(game) {
  ...
  // The scenario's own set icon (Task 5, seticon.js's scenarioIcon) before
  // the "Stage n" label - same treatment as rail.js's stage pill, and the
  // same guard: `game.scenario?.name` is the one name this app can turn
  // into an icon slug, omitted for a bare/manual game with no preloaded
  // scenario.
