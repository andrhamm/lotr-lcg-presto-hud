...
- No secrets in the repo: the wrangler step reads `CLOUDFLARE_API_TOKEN` from repo secrets and `CLOUDFLARE_ACCOUNT_ID` from a repo variable; the step is `continue-on-error: false` (a failed Cloudflare publish must show red) but placed AFTER the GitHub Pages artifact upload so the existing deploy is never blocked by it.
...
Needs a Cloudflare API token with **Account · Cloudflare Pages · Edit**, **Zone · DNS · Edit** and **Zone · Dynamic Redirect · Edit** on `lotrlcg.app` and `lotr.quest`, in `~/.zshenv` as `CF_PAGES_TOKEN`, and the same token added by the user to the GitHub repo as secret `CLOUDFLARE_API_TOKEN` plus variable `CLOUDFLARE_ACCOUNT_ID` = `31d0a323fcd472daf6c82573b6010918`.
