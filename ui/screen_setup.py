...
class SetupScreen:
    ...
    def draw(self, hw, game, pal):
        ...
        d.set_pen(pal.bg)
        d.clear()
        text_center(d, pal, "New game", 240, 16, DISPLAY, pal.gold)
        if len(self.threats) > 1:
            text_left(d, pal, "tap a row to set the first player", 24, 52, BODY, pal.dim)
        ...
        for i, t in enumerate(self.threats):
            ...
            text_left(d, pal, "P%d" % (i + 1), 30, y + 19, DISPLAY, pal.tan)
            if i == self.first:
            ...
            if len(self.threats) > 1:
                ...
                text_center(d, pal, "x", rm.x + 24, rm.y + 12, DISPLAY, pal.no_fg)
        ...
        if len(self.threats) < MAX_PLAYERS:
            ...
            text_center(d, pal, "+ Add player", 240, y + 15, BODY, pal.tan)
